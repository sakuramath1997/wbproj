 ...
 *
 * wbel-spec v8 §7 / wbelx-spec v4 §7 に準拠した破壊的操作。
...
import { computeState, getActiveStrokes, getActiveOverlays, backgroundStateToSnapshotBGEvent } from './state-machine';
import { eventToJsonl } from './serializer';

// ========================================
// DI 型定義（Core 層から外部に委譲）
// ========================================

/** Minimize が必要とする外部依存の注入インターフェース */
...
export function minimizeWbelx(
  ...
  if (state.background !== null) {
    const bgEvent = backgroundStateToSnapshotBGEvent(
      state.background, ts, sid, deps.generateBgOpId(),
    );
    if (bgEvent) {
